...
> 前端用**纯静态 Web（默认渲染；PreText 排版层依 Day 7 决策暂缓至后续，已预留 `renderItemText()` 接缝）**把周时间线画出来；数据落地走**腾讯文档智能表 API**；图片/表格识别走**百度智能云 OCR**（MVP 用户自备 Key 前端直连，阶段二 Node/Python 代理）；无自建完整后端，Node/Python 后端列入阶段二。
